...
const DarkLightTemplate = () => {
    ...
    const bis = {
        ...
        "imageWithOverlay": {
            ...
            "image": "https://mydukaan.io/_next/static/media/banner-bg-grid.9316afcddfcb78d705d0f20c456dd4be.svg",
            "CtaName": "Know More",
    ...
    const getBusiness = async () => {
        ...
        setIsLoading(false);
        setFaq(bis?.faq?.map((item) => ({ ...item, show: false })))
    ...
    return (
        ...
        >
            <div className="">
                ...
                <AwesomeSlider animation="foldOutAnimation" style={{ "--slider-height-percentage": width < 500 ? "30vh" : width < 1024 ? "50vh" : "80vh", "--control-bullet-active-color": primary, "--control-bullet-color": "#6a6a6a", "--organic-arrow-color": primary, "--loader-bar-color": primary }}>
                    {
                        business?.banners.map((item, index) => (
                ...
                <div className="my-8 py-12 px-4 text-base  relative overflow-hidden" style={{ background: `linear-gradient(to left,${from} , ${to} )` }}>
                    <div className="flex flex-col items-center gap-4 max-w-[800px] text-center mx-auto px-4 relative z-[100]">
                        <h1 className="text-3xl font-bold">{business?.imageWithOverlay?.title}</h1>
                        <p className="text-xl">{business?.imageWithOverlay?.description}</p>
                        <Link to={business?.imageWithOverlay?.CtaLink} className="px-4 mt-4 inline-block transition-all duration-300 rounded-sm py-2 hover:-translate-y-[10%] font-bold" style={{ background: primary, color: innerText }}>{business?.imageWithOverlay?.CtaName}</Link>
                    </div>

                    <div className="absolute top-0 left-0 w-full h-full z-10">
                        <img src={business?.imageWithOverlay?.image} className="object-cover w-full h-full scale-150" alt="" />
                    </div>
                </div>
                            ...
                            Array.from({ length: 3 }).map((item, index) => (
                                business?.dataSlate[`value${index + 1}`] ?
                                    <div key={index} className={`sm:basis-[40%] text-base`}>
                                        ...
                                        <p className="text-2xl mt-4 text-muted ">{business?.dataSlate[`dataName${index + 1}`]}</p>
                                    </div> : null
                            ))
                        }
                    </div>
